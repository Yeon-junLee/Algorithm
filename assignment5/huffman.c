...
tNode* make_huffman_tree(int ch_freq[]) {
	...
	for (c = 0; c < 256; c++) {
		tNode* node = newNode(c, ch_freq[(int)c]);
		success = heapInsert(heap, node);
		if (success == 0)
			break;
	}
	...
	while (heap->last > 0) {
		...
		tNode* new = newNode(20, (min1->freq) + (min2->freq));
		new->left = min1;
		new->right = min2;

		success = heapInsert(heap, new);
		if (success == 0)
			break;
	}

	tNode* result = heapDelete(heap);
...
int encoding(char* codes[], FILE* infp, FILE* outfp) {
	...
	while ((ch = fgetc(infp)) != EOF) {
		fwrite(codes[(int)ch], strlen(codes[(int)ch]), 1, outfp);
		total = total + strlen(codes[(int)ch]) * sizeof(char);
	}
